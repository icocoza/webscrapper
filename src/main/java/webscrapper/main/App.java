...
public class App 
{
    ...
    private static void testElasticSearch() {
    	try {
			...
			SearchResponse res = ElasticTransportMgr.getInst().matchSearch("food", "foodnut", "title", "오뚜기");
    ...
    private static void testElasticSearchClient() {
    	try {
			...
			//SearchResponse res = ElasticTransportMgr.getInst().matchSearch("food", "foodnut", "title", "새우깡");
			//System.out.print(res.getHits().getTotalHits()+"");
			System.out.print("done");
		} catch (Exception e) {
			e.printStackTrace();
